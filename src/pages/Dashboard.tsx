...
export default function Dashboard() {
  ...
  const confirmDeleteProject = async () => {
    ...
    try {
      ...
      setErrorMsg(
        err.message || "Error deleting project. Insufficient permissions.",
      );
      ...
      unsubscribeTraffic = onSnapshot(
        ...
        (error) =>
          handleFirestoreError(
            error,
            OperationType.LIST,
            `projects/${activeProject.id}/analytics`,
          ),
      );
    } else {
      setActivities([]);
    ...
    <div className="p-6 lg:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            onChange={(e) =>
              setActiveProject(
                activeProjects.find((p) => p.id === e.target.value) || null,
              )
            }
      ...
      {isProfileIncomplete && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-r from-theme-primary/20 to-purple-500/20 border border-theme-primary/30 p-5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
        >
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-theme-muted text-sm mt-1">
                Help TriMatrix AI provide better, more tailored suggestions and
                content by telling us about your business.
              </p>
            </div>
          </div>
          <Link
            to="/settings"
            className="shrink-0 bg-white/10 hover:bg-white/20 text-theme-main px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center"
          >
      ...
      {/* TriMatrix Nexus AI Panel */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="glassy-neumorphic rounded-2xl p-6 relative overflow-hidden bg-gradient-to-br from-theme-surface to-black border border-theme-primary/20"
      >
        <div className="absolute right-0 top-0 w-64 h-64 bg-theme-primary/10 rounded-full blur-3xl"></div>
        <div className="flex flex-col md:flex-row gap-6 relative z-10">
          <div className="flex-1">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-all flex items-center border ${
                isAutoOptimizing
                  ? "bg-theme-primary/20 text-theme-primary border-theme-primary shadow-[0_0_15px_rgba(75,17,242,0.3)]"
                  : "bg-black/40 text-theme-muted border-white/10 hover:border-white/20"
              }`}
            >
          ...
          <div className="flex-1 grid grid-cols-2 gap-4">
            <div className="bg-black/30 rounded-xl p-4 border border-white/5">
              <div className="text-xs text-theme-muted mb-1 flex items-center">
            ...
            <div className="bg-black/30 rounded-xl p-4 border border-white/5">
              <div className="text-xs text-theme-muted mb-1 flex items-center">
                <Target className="w-3 h-3 mr-1" /> Global Sentiment
              </div>
              <div className="text-2xl font-bold text-theme-primary">
                {globalSentimentScore}/100
              </div>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <div className="space-y-2">
              {predictiveInsights.map((insight, idx) => (
                <div
                  key={idx}
                  className="flex items-start bg-black/20 p-3 rounded-lg border border-white/5 text-sm text-theme-muted"
                >
                  <span className="text-theme-primary mr-2 font-bold select-none">
                    &gt;
                  </span>
                  {insight}
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            value:
              trafficData.length > 0
                ? `$${trafficData.reduce((acc, curr) => acc + (curr.spend || 0), 0).toLocaleString()}`
                : "$0",
            change: "+12.5%",
            icon: Target,
          ...
          },
          {
            ...
            value:
              trafficData.length > 0
                ? `${Math.round(trafficData.reduce((acc, curr) => acc + (curr.traffic || 0), 0) / trafficData.length).toLocaleString()}`
                : "0",
            change: "+4.2%",
            icon: TrendingUp,
          ...
          >
            ...
            <div className="mt-4 flex items-baseline">
              ...
              {kpi.change && (
                <p
                  className={`ml-2 flex items-baseline text-sm font-semibold ${kpi.change.startsWith("+") ? "text-emerald-400" : "text-rose-400"}`}
                >
                  {kpi.change}
                </p>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glassy-neumorphic rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="text-theme-muted hover:text-theme-main transition-colors">
          ...
          <div className="h-80">
            {trafficData.length === 0 ? (
              <div className="w-full h-full flex flex-col items-center justify-center border border-dashed border-white/10 rounded-xl bg-black/20">
                ...
                <p className="text-theme-muted text-sm max-w-sm text-center mt-1">
                  Connect your platforms in Settings or click "Analyze Real
                  Data" to run an AI simulation based on your business profile.
                </p>
              </div>
            ) : (
        ...
        <div className="glassy-neumorphic rounded-2xl p-6 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-theme-primary text-sm hover:underline">
              View All
            </button>
          </div>
          <div className="flex-1 space-y-5 overflow-y-auto pr-2">
            {activeProjects.map((project) => (
              <div
                key={project.id}
                className="group p-4 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer"
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h4 className="font-medium text-theme-main group-hover:text-theme-primary transition-colors">
                      {project.name}
                    </h4>
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${project.status === "Active" ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}
                  >
                    {project.status}
                  </span>
                </div>
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-theme-muted text-sm border-l-2 border-red-500 pl-3">
                  This action cannot be undone. All project-specific data
                  including activities, social posts, generated content, and
